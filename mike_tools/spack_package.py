...
#spack edit wrf
#copy contents
# cd to root WRF dir, and run:
#spack dev-build wrf@4.2+adios2
import os
import glob
import re
import time
...
class Wrf(Package):
    ...
    variant(
        "adios2",
        default=False,
        description="Parallel IO support through ADIOS2 library",
    )
    ...
    depends_on("adios2@master +python", when="+adios2")
    depends_on("netcdf-c")
    ...
    def setup_build_environment(self, env):
        ...
        if "+adios2" in self.spec:
            ...
            adios2_config =  Executable(os.path.join(self.spec["adios2"].prefix.bin, 'adios2-config'))
            ADIOS2INC = adios2_config('--fortran-flags', output=str)
            env.set("ADIOS2INC", ADIOS2INC)
            ADIOS2LIB = adios2_config('--fortran-libs', output=str)
            env.set("ADIOS2LIB", ADIOS2LIB)
