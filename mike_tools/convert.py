...
def r_metadata(adios2f, attrs):
    ...
    dim_lens = {key[5:]:int(value["Value"]) for key, value in attrs.items() if key.startswith("_DIM_")}
...
def convert(input_file, output_file, diskless=False):
    ...
    adios2f, netcdff = open_files(input_file, output_file, parallel=parallel, comm=comm, diskless=diskless)
